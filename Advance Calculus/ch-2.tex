...
\begin{enumerate}[wide,label = \textbf{\arabic*.}]
	...
	\begin{enumerate}[label = $\bullet$]
		...
		whose length are $\infty$ and boundary
		
		$$\begin{cases}
			\partial(a,+\infty) = \partial(a,+\infty) = \{a\}\\
			\partial(-\infty,b) = \partial (-\infty,b) = \{b\}
		\end{cases}$$
	...
	\end{enumerate}
	\item Convex sets in $\R^k$(In vector space)
		
		\begin{defn}
			$S \subseteq \R^k$ is convex if $\forall~ x,y \in S,~\overline{xy}$ is the line segment joining $x ~\&~ y$
		\end{defn}
		
		Given $p,q \in \R^k$, The parametric equation of
		
		\begin{center}
			$\overleftrightarrow{pq} = \{p + t(q-p) ~|~t \in \R\}$: the line in $\R^k$ throw $p$ and $q$\\
			$\overrightarrow{pq} = \{p + t(q - p)~|~ t \geq 0\}$: the ray in $\R^k$ throw p and $q$\\
			$\overline{pq} = \{p+t(q-p)~|~0 \leq t \leq 1\}$: the line segement throw p and q
		\end{center}
		
		\begin{eqnarray*}
			x \in \overline{pq} \Leftrightarrow x &=& p+t(q-p) \text{ for some } t \in [0,1]\\
			&=& p + tq - tp\\
			&=& (1 - t)p + tq\\
			&=& \alpha p + \beta q, \text{where $\alpha$ $\beta$  $\geq$ 0 and $\alpha + \beta = 1$}
		\end{eqnarray*}
		
		and we called $\alpha p + \beta q$ convex combination of $p$ and $q$.
		
		Note that all open balls, closed balls, intervals are convex in $\R^k$
		
		\begin{tcolorbox}
			Given $B(p,r)$, $\forall x,y \in B(p,r)$ and $\alpha , \beta \geq 0, \alpha + \beta = 1$
			
			\begin{eqnarray*}
				\norm{\alpha x + \beta y - p} &=& \norm{\alpha x + \beta y - (\alpha + \beta)p}\\
				&=& \norm{\alpha x - \alpha p + \beta y - \beta p}\\
				&\leq& \norm{\alpha x - \alpha p} + \norm{\beta y - \beta p}\\
				&=& \alpha \norm{x - p} + \beta \norm{y - p}\\
				&<& \alpha r + \beta r = (\alpha + \beta)r = r
			\end{eqnarray*}
			
			$\therefore \alpha x + \beta y \in B(p,r \therefore B(p,r))$ is convex
		\end{tcolorbox}
	...
	\item $\Z$ is closed in $\R$ and every point in $\Z$ is isolated e.g.$\{n\pi ~|~ n \in \Z\}$, $\because \R - \Z = \bigcup^{\infty}_{n = -\infty}(n-1,n)$ is open $\implies \Z$ is close. or $\Z' = \emptyset \subseteq \Z,~\therefore \Z$ is close.
...
\begin{enumerate}[wide]
	\item Let $X$ be a topology space and $K \subseteq X$ be a finite set. Then $K$ is compact. $\because$ Given an arbitrary open covering $\{U_{\alpha} ~|~ \alpha \in I\}$ of $K$, i.e. $K \subseteq \bigcup_{\alpha \in I}U_{\alpha} \implies \exists \alpha_i \in I \ni x_i \in U_{\alpha_i},1 \leq i \leq n \implies K = \{x_1,\cdots,x_n\} \subseteq \bigcup_{i = 1}^{n}U_{\alpha_i} \therefore K$ is compact.
...
\textbf{Recall.} bounded set in a metric space, specially, in $\R^k~X:$ metric space

\begin{enumerate}[label = $\star$]
	\item $S \subseteq X$ is bdd if $\exists p \in X$ and $r > 0 \ni S \subseteq B(p,r)$
	\item In $\R^k,~S \subseteq \R^k$ is bounded of $\exists M > 0 \ni \norm{x} \leq M \forall x \in S$
	
	$\because S \subseteq B(p,r) \implies \forall x_0 \in \R^k, \exists R_{x_0} > 0 \ni S \subseteq B(x_0,R^{x_0})$ for some $p \in \R^k$ and $r > 0$
	
	In particular, $S \subseteq B(0,M) = \{x \in \R^k~|~ \norm{x} < M\}$ for some $M > 0$
\end{enumerate}
...
\begin{thm}
	Let $X$ be a metric space. Then $X$ is compact \textbf{ if and only if }$ \forall$ collection $\{K_{\alpha}\}$ of closed subsets of $X$ with FIP(finite intersection property, i.e. $\forall \alpha_1,\cdots,\alpha_n \in I,K_{\alpha_1} \cap \cdots \cap K_{\alpha_n} \neq \emptyset$), $\bigcap_{\alpha \in I}K_{\alpha} \neq \emptyset$
\end{thm}

\begin{proof}$ $
	
	$(\Rightarrow)$ Suppose $X$ is compact. Given a collection $\{K_{\alpha}\}$ of closed subset of $X$ with FIP. We must prove that $\bigcap_{\alpha \in I}K_{\alpha} \neq \emptyset.$ If $\bigcap_{\alpha \in I}K_{\alpha} = \emptyset$, then $X = X - \bigcap_{\alpha \in I}K_{\alpha} = \bigcup_{\alpha \in I}(X - K_{\alpha}) \implies \exists \alpha_1,\cdots,\alpha_m \in I \ni X = \bigcup^n_{i = 1}(X - K_{\alpha_i}) = X - \bigcap^n_{i=1}K_{\alpha_i} \implies \bigcap^n_{i = 1}K_{\alpha_i}K_{\alpha_i} = \emptyset (\rightarrow\leftarrow)$ to FIP.
	
	$(\Leftarrow)$ Suppose the condition holds. To prove that $X$ is compact. If $X$ were not compact, them $\exists$ an open covering $\{U_{\alpha}\}_{\alpha \ni I}$ of $X$ which has no finite subcovering. Let $K_{\alpha} = X - U_{\alpha}, \alpha \in I$. Then $\{K_{\alpha}\}_{\alpha \in I}$ is a collection of closed subsets.
	
	Claim it satisfies FIP. GIven $\alpha_1,\cdots,\alpha_n \in I,~\bigcap_{i=1}^nK_i = \bigcap^n_{i=1}(X - U_{\alpha_i}) = X - \bigcup_{n=1}^nU_{\alpha_i}$
	
	$$\therefore \bigcap_{i=1}^nK_i = \emptyset \Leftrightarrow X - \bigcup^n_{i=1}U_{\alpha} = \emptyset \Leftrightarrow X = \bigcup^n_{i=1}U_{\alpha_i} (\rightarrow\leftarrow)$$
	
	$$\therefore \bigcap^n_{i=1}K_{\alpha_i} = \emptyset$$
	
	By assumption, $\bigcap_{\alpha \in I}K_{\alpha} \neq \emptyset$, i.e. $\bigcap_{\alpha \in I}(X - U_{\alpha}) \neq \emptyset \implies \X - \bigcup_{\alpha \in I}U_{\alpha} =\emptyset \implies \bigcup_{\alpha \in I}U_{\alpha} \neq X(\rightarrow\leftarrow)$
	
	$\therefore X$ must be compact.
...
\begin{defn}
	...
	\begin{enumerate}[wide]
		\item $X$ is first if every point of $X$ has a countable nbh system(or base), i.e. $\forall p \in X \exists$ a countable collection $\{V_n ~|~ n \in \N\}$ of nbh of $p \ni \forall$ nbh $U$ of $p,\exists~ n \in \N,~ V_n \subseteq U$
...
\begin{rmk*} $ $
	\begin{enumerate}
		\item Every finite topological space is 1,2 countable and separable.
		\item Every $2^{nd}$ countable topology space $X$ is of $1^{st}$ countable, but not converse.\\
		$\because$ Let $\mathscr{B} = \{B_1,B_2,\cdots\}$ be a countable base for $X$. Given $p \in X$, let $\mathscr B_p = \{B_n ~|~ p \in B_n\}$ Then $\mathscr B$, so $\mathscr B_p$ is countable and is a collection of open set in $X$ containing of $p$.\\
...
\begin{rmk*}
	An equivalent description of base in a topological space. Let $X$ is a topological space, $\B$ is a collection of open sets in $X$.
	
	\begin{eqnarray*}
		\B \text{ is a base for } X  &\Leftrightarrow & \forall \text{ open set }U \in X ~\&~p\in U\\
		&&\exists B \in \B \ni p \in B \subseteq U
	\end{eqnarray*}
	
	\begin{proof}
		$(\Rightarrow)$ Suppose $\B$ is a base. Given an open set $U$ in $X$ and $p \in U$. Since $\B$ is a base, $U = \bigcup_{\alpha \in I}B_{\alpha}$, for some index set $I$ and $B_{\alpha} \in \B,~\forall \alpha \in I$. $p \in U \implies p \in \bigcup_{\alpha \in I}B_{\alpha} \implies p \in B_{\alpha_0}$ for some $\alpha_0 \in I \implies p \in B_{\alpha_0} \subseteq U$ and $B_{\alpha_0} \in \B$
		
		$(\Leftarrow)$ Suppose the condition holds. Given an open set $U$ in $X$. By assumption, $\forall x \in U \exists B_x \in \B \ni x \in B_x \subseteq U$. $\therefore U = \bigcup_{x \in U}B_x$, i.e. $U$ is a union of $\{B_x ~|~ x \in U\} \subseteq \B$, $\therefore \B$ is a base for $X$.
	\end{proof}
\end{rmk*}
...
\begin{rmk*}
	$\R^k$ is of second countable which has a countable base $\B = \{B(x,r) ~|~ x \in \Q^k,~r \in \Q^+\}$
\end{rmk*}
